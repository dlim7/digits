import React from 'react';
import { Grid, Image, Icon, Header } from 'semantic-ui-react';

/** A simple static component to render some text for the landing page. */
class Landing extends React.Component {
  render() {
    return (
        <div className='digits-landing-background'>
          <Grid container stackable centered columns={3}>

            <Grid.Column textAlign='center'>
              <Icon size='huge' name='users' inverted/>
              <Header as='h1' inverted>Multiple Users</Header>
              <Header as='h3' inverted>This address book enables any numbers of users to login and save their business contacts. You can only see the contacts that you have created</Header>
            </Grid.Column>

            <Grid.Column textAlign='center'>
              <Icon size='huge' name='file text' inverted/>
              <Header as='h1' inverted>Contact Details</Header>
              <Header as='h3' color='green'>This address book enables any numbers of users to login and save their business contacts. You can only see the contacts that you have created</Header>
            </Grid.Column>


            <Grid.Column textAlign='center'>
              <Icon size='huge' name='checked calendar' inverted/>
              <Header as='h1' inverted>Timestamped Notes</Header>
              <Header as='h3' inverted>This address book enables any numbers of users to login and save their business contacts. You can only see the contacts that you have created</Header>
            </Grid.Column>


          </Grid>
        </div>
    );
  }
}

export default Landing;
